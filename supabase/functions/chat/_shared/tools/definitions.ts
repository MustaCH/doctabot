    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          zone: { type: "string", description: "Zona de Córdoba Capital: Ruta 20, Nueva Córdoba, Centro, Alberdi, Alta Córdoba, General Paz, Zona Sur, Zona Norte. Acepta VARIAS separadas por coma (ej: 'Nueva Córdoba, Centro') y matchea cualquiera (OR)." },
          ...
          property_type: { type: "string", description: "Tipo de propiedad: Departamento, Casa, Terreno, Local, Oficina, etc. Acepta VARIOS separados por coma (ej: 'Casa, Departamento') y matchea cualquiera (OR)." },
          exclude_zones: { type: "string", description: "Zona(s) a EXCLUIR, separadas por coma (ej: 'Nueva Córdoba, Centro'). Usalo cuando el cliente pide 'en todos lados MENOS X'." },
          exclude_neighborhoods: { type: "string", description: "Barrio(s) (zone_neighborhood) a EXCLUIR, separados por coma." },
          min_price: { type: "number", description: "Precio mínimo. Debe ser <= max_price. Si no pasás currency, se asume USD." },
          max_price: { type: "number", description: "Precio máximo. Si no pasás currency, se asume USD." },
          currency: { type: "string", description: "Moneda: USD o ARS. Default cuando hay min_price/max_price: USD (el mercado de venta opera en dólares). Pasá ARS explícito para presupuestos en pesos (típico en alquileres)." },
          ...
          exclude_office: { type: "string", description: "Oficina a EXCLUIR (NOT ILIKE). Ej: 'Docta' cuando el agente pide 'las de otras oficinas'." },
          docta_first: { type: "boolean", description: "Priorizar las propiedades de RE/MAX Docta al tope de los resultados (default true). Poné false SOLO si el agente pide explícitamente ver las de otras oficinas (combinalo con exclude_office='Docta')." },
          only_active: { type: "boolean", description: "Solo publicaciones activas (listing_status='active' o sin estado registrado). Default true. Poné false SOLO si el agente pide explícitamente incluir publicaciones dadas de baja." },
          offset: { type: "integer", description: "Desde qué resultado empezar (paginación). Si el agente pide 'mostrame más' de la misma búsqueda, repetí los filtros con offset = resultados ya mostrados." },
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          to: { type: "string", description: "Destinatario(s): emails PUROS separados por coma (ej: 'ana@mail.com, luis@mail.com'). SIN display name — 'Nombre <mail@x.com>' es inválido y el envío se rechaza. Máximo 20 direcciones." },
          subject: { type: "string", description: "Asunto del email" },
          body: { type: "string", description: "Cuerpo del email en TEXTO PLANO, NO uses HTML (se envía como text/plain)." },
          cc: { type: "string", description: "Copia (CC), opcional: mismo formato que 'to' (emails puros separados por coma, sin 'Nombre <mail>')." },
        },
