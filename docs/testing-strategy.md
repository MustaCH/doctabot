...
2. **Inyección PostgREST en `save_property_to_client`** (86aj0p5by) — el fix (`.ilike` de columna única) sigue sin test del escenario de inyección. El bloqueo histórico (no había mock del cliente Supabase) **ya no existe**: `executor.search.test.ts` estableció el patrón. Acción pendiente: extenderlo al case de inyección. Nota: `executor.save-property.test.ts` (bug 86ak29y3q, 2026-08-18) ya cubre el case con mock por-tabla — sirve de base: propiedad borrada por el scraper → error claro sin FK 23503, upsert con `onConflict` intacto, y `safeDbError(23503)` accionable.
