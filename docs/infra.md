...
- 🟠 **Security advisors (DB):** funciones `SECURITY DEFINER` ejecutables por `anon`/`authenticated` vía RPC, heredadas del clone. **Parcialmente resuelto 2026-08-18 (ticket 86aj9w5jm):** las 3 `admin_*` (`admin_user_reports`, `admin_engagement_report`, `admin_time_stats`) quedaron cerradas con la migración `20260818120000_close_admin_rpcs_privilege_escalation` — guard interno `assert_admin_rpc_access()` (pasa `service_role` o `super_admin`, si no `forbidden` 42501) + `REVOKE EXECUTE FROM PUBLIC/anon/authenticated` + `GRANT` a `service_role`. Ojo: el guard acepta `auth.role()='service_role'` porque `admin-stats` llama con la service key (`auth.uid()` es NULL ahí). Verificado en prod: `authenticated` → permission denied; camino service_role → las 3 devuelven datos. **Pendiente:** `has_role`, `validate_invitation_code*`, `cleanup_old_logs` siguen ejecutables por `authenticated` (menor riesgo — no exponen datos cross-tenant), y "leaked password protection" sigue OFF en Auth. Revisar con Architect.
- ✅ **Código de invitación DOCTA1 desactivado (2026-08-18, ticket 86aj9w5k9).** Estaba en texto plano en el historial público del repo. Migración `20260818121500_deactivate_docta1_limit_invitation_codes`: `is_active=false` para DOCTA1 + columnas `max_uses`/`uses`/`expires_at` en `invitation_codes`, enforced en `validate_invitation_code_v2` (expirado/agotado → `inactive`, que el front ya muestra como "ya no está vigente"; un `valid` incrementa `uses`) y en la v1. **El código vigente NO está en git**: se generó random directo en la DB (20 usos, 90 días); Nacho lo tiene. Para generar otro: `INSERT INTO invitation_codes (code, is_active, max_uses, expires_at) VALUES ('<random>', true, <n>, now() + interval '<n> days');` vía SQL ad-hoc, nunca en una migración.
...
> **Estado de deploy al 2026-08-20 (sprint rediseño "Carbón & Vidrio"):** `chat` **v54** (tarjetas rotuladas, error con `executedTools` + reintento, eventos SSE `step`). PWA redeployada desde `c13a2bb` (auto-deploy por push + rebuild manual). Gotcha de la API de Dokploy: `application.one` devuelve los deployments **truncados a los más viejos** — para ver los últimos usar `deployment.all?input={"json":{"applicationId":...}}`.
